...
export default function EmployeeDetailAdmin() {
   ...
   const fetchData = async () => {
      try {
         if (!targetId) { alert('No employee ID specified'); return; }
   ...
   const canManage = ['Admin', 'HR', 'AGM', 'SuperAdmin'].includes(currentUser?.role);
   const canSeeCredentials = ['SuperAdmin', 'Admin', 'HR'].includes(currentUser?.role);
   ...
   return (
      <>
         <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-10 sm:pb-20">
               ...
               <div className="flex items-start sm:items-center gap-3 sm:gap-5">
                  ...
                  <div>
                     <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
                        {employee.name}
                        {employee.employeeId && <span className="text-slate-300 font-light ml-2">({employee.employeeId})</span>}
                     </h1>
                     <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                           {id ? 'Employee File' : 'My Personal Profile'} • {employee.employeeId || 'ID PENDING'}
                        </span>
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${
                           employee.kycStatus === 'Approved' ? 'bg-emerald-50 text-emerald-600' :
                           employee.kycStatus === 'Pending'  ? 'bg-amber-50 text-amber-600 animate-pulse' :
                        ...
                        {activeTab === 'overview' && (
                           !editMode ? (
                              ...
                              <div className="flex flex-wrap gap-2 sm:gap-3">
                                 <button onClick={() => setEditMode(false)} className="px-6 py-3.5 bg-slate-100 text-slate-600 rounded-2xl font-bold text-sm">
                                    Cancel
                                 </button>
                                 <button
            ...
            {/* ── Tab Bar (only when viewing a specific employee, not own profile) ── */}
            {id && (
               <div className="flex items-center gap-1 bg-white border border-slate-100 rounded-2xl p-1.5 shadow-sm w-fit">
                  ...
                  {canSeeCredentials && (
                     ...
                     >
                        <KeyRound size={15} /> Assets & Credentials
                     </button>
                  )}
               </div>
            )}
            ...
            {(activeTab === 'overview' || !id) && (
               <div className="space-y-6 sm:space-y-8">
                        ...
                        <div className="pt-6 border-t border-slate-50 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                           <div>
                              ...
                              <p className="text-xl font-black text-slate-900 mt-1">
                                 ₹{employee.salaryStructure?.baseSalary || 0}
                  ...
                  {canManage && id && <ReuploadRequestsPanel employeeId={id} />}

                  {/* KPI Management & History */}
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                     ...
                     <div className="lg:col-span-2 bg-white rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 lg:p-10 shadow-sm border border-slate-200/60">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-6 sm:mb-8">
                           ...
                           <div className="px-4 py-2 bg-slate-50 rounded-xl text-[10px] font-black uppercase text-slate-400 tracking-widest">
                              {history.length} Entries Record
                           </div>
                        </div>
